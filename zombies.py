            ...
            if self.zombie_spawn_index<len(zombies_pairs):
                if time.time()-self.last_zombie_spawn_time>=zombie_time[self.zombie_spawn_index]:
                    for i in range(zombies_pairs[self.zombie_spawn_index]):
                        ...
                        self.zombies.append({"x": zombie_x, "y": zombie_y, "rect": zombie_rect,"points": zombie_points,"zombie_pictures":zombie_images,"index": 0, "attack_index":0, "picture_time": time.time(), "line": ((zombie_y-20)//100)+1})
        ...
        for pea in cells.peas:
            for zombie in self.zombies:
                if zombie["rect"].colliderect(pea["rect"]):
                    ...
                    #if zombie["points"]<=6 and zombie["zombie_pictures"]==self.zombies_pictures:
